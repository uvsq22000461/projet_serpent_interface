...
#Fonction réinitialiser les variables pour rejouer
def init_variables():
...
#Fonction créer la page du menu ( frame = cadre)
def fenetre_menu():
    ...
    police1=tkFont.Font(cadre_menu, size=15, family='Courier',weight = 'bold' )   #  définition de la police de caractères ( family (police), bold (caratère gras))
    ...
    b3 .place(x = 400, y = 100)     # placement de du bouton en fonction des coordonnées x et y
...
# Fonction enregistrer le pseudo dans la variable nom
def pseudo():
    global nom
    nom = nom_entr.get()

# Fonction création de la page du jeu 
def fenetre_jeu():
    global canvas,  text_score,  cadre_jeu,  nom
    cadre_menu.destroy()     # détruit la fenetre du menu ( ou plus exactement détruit le Frame du menu
    cadre_jeu = tk.Frame(racine, bg="chartreuse3", width=L, height=H+30)  # Création d'un nouveau  cadre (Frame) pour mettre les widget du jeu
    cadre_jeu.grid()    
...
def choix_fin(fin):
    ...
    if fin ==3 :       # si choix 3 ( bouton Quitter ) detruit la racine du jeu
        racine.destroy()
    if fin==2:           # si choix 2 ( bouton Menu ) detruit le cadre de la fenêtre du jeu, réinitialise les variables et lance de nouveau la foncton 'fenetre_menu() ' 
        cadre_jeu.destroy()
        init_variables()
        fenetre_menu()
    if fin==1:      # si choix 1 ( bouton Nouvelle Partie ) detruit le cadre  de la fenêtre de jeu, réinitialise les variable et relance la fonction 'fenetre_jeu)
        cadre_jeu.destroy()
        init_variables()
        fenetre_jeu()      
        
# Fonction création du décor du jeu
def creer_pierres():
     ...
     for i in range (1,  L//30):     # boucle pour la création des pierres de la bordure du haut
         canvas.create_rectangle( x0,y0,x1 , y1, fill=couleur_pierre1)    # création du carré exterieur des pierres
         canvas.create_rectangle( x0+5,y0+5,x1 -5, y1-5, fill=couleur_pierre2, outline=couleur_pierre1, width=1)      # création de carré interrieur des pierres
         ...
         x0 = x0+30     # incrémante x pour dessiner la pierre suivante
         x1 = x1+30               
...
#Fonction creer du serpent du départ ( une tête et une partie de corps
def creer_serpent():
     global serpent
     ...
     coords_serp=[ x-20,y,x, y-20]       # création du premier élément de son corps ( un serpent avec seulement une tête c'est bizzare )
     coords_serpent.append(coords_serp)     


#Fonction creer corps de serpent quand le serpent mange une pomme    
def creer_corps():    
     global serpent
     i = len(serpent)-1   # i corespond au nombre d'élément de la liste du serpent  ( -1 car dans une liste les éléments sont repérés  à partir de 0
     x0, y0, x1, y1 = canvas.coords(serpent[i])      # récupération des coordonnées de la dernière partie du serpent
     corps = canvas.create_rectangle(x0-20, y0, x1, y1-20,  fill="LightGoldenrod3", outline="LightGoldenrod3")      # créé une nouvelle partie de corps au serpent apres les coordonnées de la derniére partie
     ...
     coords_serp=[ x0, y0, x1, y1]     # mise des coordonnées sous forme de liste
...
def creer_pomme():
    global pomme,  xp,  yp      # xp, yp coordonnéés du centre de la pomme 
...
def mouvement():
     ...
     i=len(serpent) -1   # i corespond au nombre d'élément de la liste du serpent  ( -1 car dans une liste les éléments sont repérés  à partir de 0
     # lances le fonctions suivantes pour vérifier que le serpent n'interagie pas avec le decor, lui-même ou la pomme 
     inter_pierre()
     inter_serpent()
     inter_pomme()
     
     if perdu == 0 :     #  les actions suivantes ne sont réalisées que si la variable perdu est à 0 ( pas d'interraction avec le décor ou lui-même )
        while i != 0:  
            coords_serpent[i]=coords_serpent[i-1]   # décale les coordonnées de chaque partie du serpent dans la liste  (en commencant par le dernier et l'avant edrnier ) 
            i=i-1      # decrémentation de l'indice i ( décalage de l'avanyt dernier et de l'avant avant dernier ) ect...     
        ...
        coords_serp=[ x0, y0, x1, y1]     # les met dans la liste coord_serp
        coords_serpent[0]=(coords_serp)         # remplace les coordonnées de la tête par les les nouvelles coordonées dans la liste des coordonnées des différentes partie du serpent
        ...
        canvas.create_text((L//2-20, H//2),  text=" Game over",  fill="blue",  font="Arial 30 bold")   #  si perdu passe à 1, inscritpion du texte GAME OVER       
     if grandir ==1:   
         creer_corps()
         grandir = 0
        
     canvas.after(vit, mouvement)    #  recommence le foncton ' mouvement() ' selon le réglage de vit  ( exemple vit=200, toutes les 200ms )
          
#Fonction changeant les coordonées de déplacement  
def droite(event):
...
#Fonction interaction avec les pierres
def inter_pierre():
     global coords_serpent, perdu     # coors_serpent [0][x], le [0] repère de la tête du serpent dans la liste, [x] repère des coordonées (si [0]=x0, [1]=y0, [2]=x1, [3]=y1
     if coords_serpent[0][0] <= 30 or coords_serpent[0][2] >=L-30 or coords_serpent[0][1] <= 30 or coords_serpent[0][3] >=H-30 :
         game_over()

#Fonction interaction Serpent Corps
def inter_serpent():
    global coords_serpent,  perdu
    i= len(coords_serpent)
    for i in range (1, i):     #  les coordonnées commencant par [0] sont celle de la tête du serpent et celle commancant par [i] sont celle du reste de sont corps
...
#Foncition interaction Serpent Pomme
def inter_pomme():
...
def compter_score():
    ...
    str_score = "Score: " + str(score)       #  transforme la variable score en chaine de caractères ( pour qu'elle puisse être écrite en tant que texte dans tkinter
...
def game_over():
    ...
    perdu= 1          # passage de la variable perdu à 1, empèche la fonction "mouvement()"  d'executer les mouvements ( arrête le serpent )
...
# Fonction implémenter tableaux des scores en fonction de la vitesse
def tableau_score():
...
# Fonction Ecrire à chaque fin de partie ( game over) le nom et score dans le fichier texte correspondant
def ecrire_fichier_score():    
    with open("Score_petit.txt",  "w") as score_pet:      #  ouverture du fichier texte Score_petit en ecriture "w" en tant que score_pet
        ...
        if i <= 10 :     #   il ne doit pas y avoir plus de 1o scores dans le fichier texte ( si la longueur de i est inferieur à 10 , la valeur de i reste à i  )
            i = i
        else :     #  (  si la valeur de i est plus grande que 10, on l'impose à 10 )
            i = 10        
        for i in range (0, i):
            score_pet.write(liste_score_petite[i][0] +":")         #  écriture du nom    ( remarque l'ouverutre et l'ecriture dans un fichier texte efface le contenu précdent du fichier )
            score_pet.write(str(liste_score_petite[i][1]) +"\n")        #  écriture du score et retour à la ligne (\n) pour l'ecriture du nom suivant    
...
#Fonction lire les fichiers textes score pour remplir les liste_score_petite, moyenne, grande
def lire_fichier_score():    # with open permet de d'ouvrir le fichier, l'avantage c'est qu'il se fermera automatiquement à la fin du with ( indentation )
    try:  # essai d'ouvir le fichier Score_petit en lecture ( si il existe les instruction suivante seront executer, sinon il y aura une IOError
         with open("Score_petit.txt",  "r") as score_pet:        # ouverture du fichier Score_petit en lecture "r"
            for ligne in score_pet:      
                liste=ligne.split(":")        # récuperation de chaque chaines de caractères qui sont séparer par ( : ) ( pour chaque ligne du fichier )
                nom =(liste[0])                # la première chaine de caractère ( liste[0] est mis dans la variable nom
                score=int(liste[1])           #la deuxième chaine de caractère  (liste[1] est mis dans la variable score sous forme d'un entier ( int )
                liste_score_petite.append([nom,score])          # ajoute nom et score à liste_score_petite
    except IOError:     #    Si le fichier n'existe pas, il y aura une IOError,, cette ligne permet au programme de ne pas s'interrompre et excute la ligne suivante
        with open("Score_petit.txt",  "w") as score_pet: pass    #   ouverture du fichier Score_petit en ecriture ( ouverture en ecriture = création de fichier )  : pass  = pas d'action executé en suite
...
#Lancement du menu
fenetre_menu()
#recuperation des anciens scores dans les fichiers textes
lire_fichier_score()
